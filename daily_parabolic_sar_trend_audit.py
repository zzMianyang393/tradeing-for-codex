"""Frozen, regime-conditioned audit for daily Parabolic SAR direction flips."""
...
RULE_ID = "daily_parabolic_sar_trend_v1"
AF_STEP = 0.02
AF_MAX = 0.20
...
def sar_directions(bars: list[Bar], step: float = AF_STEP, maximum: float = AF_MAX) -> list[int]:
    """Return +1/-1 after each completed bar; no output uses later bars."""
    if not bars:
        return []
    direction = 1 if len(bars) == 1 or bars[1].close >= bars[0].close else -1
    sar = bars[0].low if direction > 0 else bars[0].high
    extreme = bars[0].high if direction > 0 else bars[0].low
    acceleration = step
    output = [direction]
    for index in range(1, len(bars)):
        candidate = sar + acceleration * (extreme - sar)
        if direction > 0:
            candidate = min(candidate, bars[index - 1].low)
            if index > 1:
                candidate = min(candidate, bars[index - 2].low)
            if bars[index].low < candidate:
                direction, sar, extreme, acceleration = -1, extreme, bars[index].low, step
            else:
                sar = candidate
                if bars[index].high > extreme:
                    extreme, acceleration = bars[index].high, min(maximum, acceleration + step)
        else:
            candidate = max(candidate, bars[index - 1].high)
            if index > 1:
                candidate = max(candidate, bars[index - 2].high)
            if bars[index].high > candidate:
                direction, sar, extreme, acceleration = 1, extreme, bars[index].high, step
            else:
                sar = candidate
                if bars[index].low < extreme:
                    extreme, acceleration = bars[index].low, min(maximum, acceleration + step)
...
def events_for_symbol(symbol: str, bars: list[Bar]) -> list[dict[str, Any]]:
    ...
    labels, atr, directions = label_completed_4h_bars(bars), wilder_atr(daily, 14), sar_directions(daily)
    events: list[dict[str, Any]] = []
    next_allowed = 0
    for index in range(14, len(daily) - 11):
        signal_ts = daily[index].ts + DAY_MS
        direction = "long" if directions[index - 1] < directions[index] else "short" if directions[index - 1] > directions[index] else None
        regime = regime_at_entry(labels, signal_ts)
        if direction is None or signal_ts < next_allowed or split(signal_ts) is None or atr[index - 1] is None:
            continue
        if (direction == "long" and regime != "趋势上行") or (direction == "short" and regime != "趋势下行"):
            continue
        exit_ts = next((daily[j].ts + DAY_MS for j in range(index + 1, min(index + 11, len(daily))) if directions[j] != directions[index]), None)
        event = trade_event(symbol, RULE_ID, direction, signal_ts + FOUR_HOURS_MS, bars, float(atr[index - 1]), exit_ts, 10 * DAY_MS, OOS_END, regime)
...
def build(data_dir: Path, symbols: list[str]) -> dict[str, Any]:
    ...
    return {"report_type": "daily_parabolic_sar_trend_audit", "rule_id": RULE_ID,
            "parameters": {"af_step": AF_STEP, "af_max": AF_MAX, "entry_delay_hours": 4, "stop_atr_multiple": 2.0, "max_horizon_days": 10, "friction_pct": 0.16},
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, default=Path("reports/daily_parabolic_sar_trend_audit.json"))
