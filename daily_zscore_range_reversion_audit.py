"""Fixed-card audit for daily Z-score range reversion."""
...
def ev(s,b):
 d=resample_minutes(b,1440);lab=label_completed_4h_bars(b);atr=wilder_atr(d,14);out=[];n=0
 for i in range(20,len(d)-9):
  ts=d[i].ts+DAY_MS; prior=[x.close for x in d[i-20:i]]; mu=mean(prior); sd=pstdev(prior)
  if ts<n or sp(ts) is None or not atr[i-1] or sd<=0 or (d[i].close-mu)/sd>-2.5 or regime_at_entry(lab,ts)!='震荡':continue
  ex=None
  for j in range(i+1,min(i+8,len(d))):
   if d[j].close>=mean([x.close for x in d[j-20:j]]):ex=d[j].ts+DAY_MS;break
  e=trade_event(s,'daily_zscore_range_reversion_v1','long',ts+FOUR_HOURS_MS,b,float(atr[i-1]),ex,7*DAY_MS,parse_day('2025-07-10',end=True),'震荡')
  if e:e['split']=sp(ts);e['zscore']=round((d[i].close-mu)/sd,6);out.append(e);n=e['exit_ts']+900000
 return out
def su(x):
