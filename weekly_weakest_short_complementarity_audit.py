"""Complementarity audit for the post-hoc weekly weakest-cohort short sleeve."""
...
from weekly_cross_sectional_momentum_audit import (
    SHORT_COMPONENT as WEEKLY_SHORT,
    generate_events as generate_weekly_events,
)
...
def duplication_diagnosis(metrics: dict[str, Any]) -> dict[str, Any]:
    triggers: list[str] = []
    if float(metrics["active_union_daily_return_correlation"]) > 0.35:
        triggers.append("daily_return_correlation")
    if float(metrics["monthly_return_correlation"]) > 0.50:
        triggers.append("monthly_return_correlation")
    if float(metrics["negative_day_overlap_coefficient"]) > 0.35:
        triggers.append("negative_day_overlap")
    operational_overlap = float(metrics["active_day_jaccard"]) > 0.50
    return {
        "likely_economic_duplicate": bool(triggers),
        "economic_duplication_triggers": triggers,
        "high_operational_overlap": operational_overlap,
        "interpretation": (
            "economic_duplication_risk"
            if triggers
            else "distinct_return_pattern_with_high_simultaneous_risk_usage"
            if operational_overlap
            else "distinct_return_pattern_and_limited_operational_overlap"
        ),
    }
...
def build_report(
    ...
    events_by_component[WEEKLY_SHORT] = [
        event
        for event in generate_weekly_events(data_dir, sorted(symbols), start_ts, end_ts)
        if event["component_id"] == WEEKLY_SHORT
    ]
    ...
    for comparison in COMPARISON_COMPONENTS:
        active_union = sorted(active[WEEKLY_SHORT] | active[comparison])
        common_months = sorted(set(monthly[WEEKLY_SHORT]) | set(monthly[comparison]))
        negative_weekly = {day for day, value in daily[WEEKLY_SHORT].items() if value < 0}
        negative_comparison = {day for day, value in daily[comparison].items() if value < 0}
        metrics = {
            "active_union_daily_return_correlation": pearson(
                [daily[WEEKLY_SHORT].get(day, 0.0) for day in active_union],
                [daily[comparison].get(day, 0.0) for day in active_union],
            ),
            "monthly_return_correlation": pearson(
                [monthly[WEEKLY_SHORT].get(month, 0.0) for month in common_months],
                [monthly[comparison].get(month, 0.0) for month in common_months],
            ),
            "active_day_jaccard": jaccard(active[WEEKLY_SHORT], active[comparison]),
            "negative_day_overlap_coefficient": overlap_coefficient(negative_weekly, negative_comparison),
            "active_days_weekly": len(active[WEEKLY_SHORT]),
            "active_days_comparison": len(active[comparison]),
            "negative_days_weekly": len(negative_weekly),
            "negative_days_comparison": len(negative_comparison),
            "event_overlap": event_interval_overlap(
                simulations[WEEKLY_SHORT].get("closed_positions", []),
                simulations[comparison].get("closed_positions", []),
            ),
        }
        reasons = pair_reasons(simulations[WEEKLY_SHORT], simulations[comparison], metrics)
        diagnosis = duplication_diagnosis(metrics)
        pair_id = f"{WEEKLY_SHORT}__{comparison}"
        pairs[pair_id] = {
            "left": WEEKLY_SHORT,
            "right": comparison,
            "metrics": metrics,
            "duplication_diagnosis": diagnosis,
            ...
            "combo_block_reason": "weekly weakest-short direction is post-hoc from a failed long-short rule",
        }
        if not reasons:
            retained.append(pair_id)
    return {
        "report_type": "weekly_weakest_short_complementarity_audit",
        "report_date": "2026-07-14",
        "scope": "posthoc_sleeve_common_window_complementarity",
        ...
        "methodology_notes": [
            ...
            "The weekly weakest-short sleeve is post-hoc and cannot authorize a combo simulation.",
            "No strategy rule, exit, cost, ranking, or allocation is changed.",
        ],
        "safety_gates": {
            "approved_for_paper": [],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Audit weekly weakest-short complementarity.")
    ...
    parser.add_argument("--out", type=Path, default=Path("reports/weekly_weakest_short_complementarity_audit.json"))
    parser.add_argument("--doc", type=Path, default=Path("docs/weekly_weakest_short_complementarity_audit_2026-07-14.md"))
