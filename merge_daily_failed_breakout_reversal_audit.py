"""Merge immutable failed-breakout audit batches into the full-universe report."""
from __future__ import annotations

import json
from pathlib import Path

from daily_failed_breakout_reversal_audit import RULE_ID
from daily_volatility_expansion_continuation_audit import summarize, verdict


def main() -> None:
    batches = [Path("reports") / f"failed_breakout_batch_{index}.json" for index in range(1, 5)]
    ...
    Path("reports/daily_failed_breakout_reversal_audit.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
