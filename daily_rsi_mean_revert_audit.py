"""Daily RSI mean-reversion audit.

Research-only rule:

  - use completed daily candles
  - go long when RSI(14) closes below 35
  - enter on the next daily open
  - exit on the next daily open after RSI recovers to 50, or after 10 days
...
def resample_daily(bars: list[PriceBar]) -> list[PriceBar]:
    ...
    for bar in bars:
        buckets.setdefault((bar.ts // DAY_MS) * DAY_MS, []).append(bar)
    daily: list[PriceBar] = []
...
def rsi_values(closes: list[float], period: int = RSI_PERIOD) -> list[float | None]:
    if period <= 0:
    ...
    values[period] = 100.0 if avg_loss == 0 else 100.0 - 100.0 / (1.0 + avg_gain / avg_loss)
    for index in range(period + 1, len(closes)):
        ...
        values[index] = 100.0 if avg_loss == 0 else 100.0 - 100.0 / (1.0 + avg_gain / avg_loss)
    return values
...
def generate_signals(
    ...
    for index in range(RSI_PERIOD, len(daily) - 1):
        value = rsi[index]
        if value is None or value >= ENTRY_RSI:
            continue
...
def find_exit_index(daily: list[PriceBar], entry_index: int) -> tuple[int | None, str]:
    ...
    for signal_index in range(entry_index, final_signal_index + 1):
        value = rsi[signal_index]
        if value is not None and value >= EXIT_RSI:
            return signal_index + 1, "rsi_recovery"
    return final_signal_index + 1, "time"


def simulate_trade(signal: RsiSignal, daily: list[PriceBar], entry_index: int) -> TradeEvent | None:
...
def run_audit(
    ...
    for base in bases:
        ...
        if not daily:
            skipped[base] = source
            continue
        if len(daily) < RSI_PERIOD + MAX_HOLD_DAYS + 30:
    ...
    return {
        "research_id": "daily_rsi_mean_revert",
        ...
        "parameters": {
            ...
            "entry": "next daily open after completed daily RSI below 35",
            "exit": "next daily open after completed RSI recovers to 50, or 10-day time exit",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Audit daily RSI mean-reversion events.")
    ...
    parser.add_argument("--out", type=Path, default=Path("reports/daily_rsi_mean_revert_audit.json"))
