"""Prepare mature Cohort D metadata only for a future sealed evaluation."""
...
from prospective_cohort_d_refresh_pipeline import DAY_MS, HORIZON_DAYS, identity
...
OUTPUT = REPORTS / "prospective_cohort_d_sealed_evaluation_preflight.json"
...
def audit_integrity(ledger: dict[str, Any], registry: dict[str, Any], maturity: dict[str, Any]) -> dict[str, Any]:
    issues = []
    signals, observations = ledger.get("signals", []), registry.get("observations", [])
    expected = {identity(signal): signal for signal in signals}
    seen = set()
    ...
    for observation in observations:
        ...
        signal = expected.get(observation_id)
        if signal is None:
    ...
    if maturity.get("as_of_utc", "") > ledger.get("common_data_cutoff", ""):
        issues.append("Maturity cutoff exceeds formal ledger cutoff.")
    return {"integrity_status": "valid" if not issues else "invalid", "n_issues": len(issues), "issues": issues, "observation_only": True}
...
def build(ledger: dict[str, Any] | None, registry: dict[str, Any] | None, maturity: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if not ledger or not registry or not maturity:
        return {"report_type": "prospective_cohort_d_sealed_evaluation_preflight", "observation_only": True,
                "readiness_status": "awaiting_first_published_observation", "formal_artifacts_available": False,
                "integrity": {"integrity_status": "not_applicable", "n_issues": 0, "issues": []},
                "mature_observation_count": 0, "queued_observation_count": 0, "queue": [],
                "result_evaluation_performed": False, "safety_gates": gates}
    ...
    readiness = spec.evidence_readiness(queue, int(maturity.get("as_of_ts", 0))) if integrity["integrity_status"] == "valid" else {"minimum_evidence_ready": False, "paper_review_ready": False, "observation_only": True}
    ...
    return {"report_type": "prospective_cohort_d_sealed_evaluation_preflight", "observation_only": True,
            "readiness_status": status, "formal_artifacts_available": True, "as_of_utc": maturity.get("as_of_utc"),
            "integrity": integrity, "mature_observation_count": len(mature), "queued_observation_count": len(queue), "queue": queue,
            "evidence_readiness": readiness, "result_evaluation_performed": False, "safety_gates": gates}


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--out", type=Path, default=OUTPUT)
